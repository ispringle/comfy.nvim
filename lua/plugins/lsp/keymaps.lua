...
function M.buf_set_keymaps(bufnr, type)
    ...
    for _, mode in pairs { "n", "v" } do
        buf_set_keymap(
            mode,
            "[e",
            "<cmd>lua require'plugins.lsp.diagnostics'.goto_prev({ severity_limit = 'Error' })<CR>",
            opts
        )
        buf_set_keymap(
            mode,
            "]e",
            "<cmd>lua require'plugins.lsp.diagnostics'.goto_next({ severity_limit = 'Error' })<CR>",
            opts
        )
        buf_set_keymap(mode, "[E", "<cmd>lua require'plugins.lsp.diagnostics'.goto_prev()<CR>", opts)
        buf_set_keymap(mode, "]E", "<cmd>lua require'plugins.lsp.diagnostics'.goto_next()<CR>", opts)
    end
    buf_set_keymap("n", "].", "<cmd>lua require'plugins.lsp.diagnostics'.show_line_diagnostics()<CR>", opts)
end
